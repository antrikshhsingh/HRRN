...
//   cpu.innerHTML = "CPU Utilization : " + Math.ceil((tbt / lastct) * 100) + "%";
//   outputDiv.appendChild(cpu);

  let totalWaitingTime = 0;
  output.waitingTime.forEach((element) => (totalWaitingTime += element));
  let avgWaitingTime = document.createElement("p");
  avgWaitingTime.innerHTML =
    "Average Waiting Time : " + totalWaitingTime / process;
  outputDiv.appendChild(avgWaitingTime);

  let totalTurnAroundTime = 0;
  output.turnAroundTime.forEach((element) => (totalTurnAroundTime += element));
  let avgTurnAroundTime = document.createElement("p");
  avgTurnAroundTime.innerHTML =
    "Average Turn Around Time : " + totalTurnAroundTime / process;
